...
- **실측 수치**(데이터 규모·배포 형태에 민감한 경로를 만들었을 때만): 사용한 데이터 규모 +
  경로별 소요 시간(초). 사용자용 문장으로 옮기는 것은 세션의 몫이다(`verification-ladder`).
- **환경 변화**(의존성·락파일·환경변수가 바뀌었을 때만): 무엇이 바뀌었는지 — 재설치 필요 여부 판단용
