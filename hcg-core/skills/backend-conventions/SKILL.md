...
description: 백엔드/API 작업의 포터블 컨벤션(HCG 표준) — contracts/api-spec·shared-types를 SSOT로, Next.js App Router API Routes(얇게, features actions 호출), 기능별 actions.ts에 로직, Prisma 데이터 접근, Zod 입력 검증, 변경 후 tsc/lint/build + Vitest 테스트 검증, contracts 읽기전용, 불일치는 완료 보고에 명시. API·서비스 로직 작업 시 로드.
---
...
- **데이터 접근**: Prisma 로 DB 에 접근한다(MariaDB, `provider="mysql"`). 클라이언트는 `lib/db.ts` 의 prisma 싱글턴을 import 한다(Prisma 7 드라이버 어댑터 `@prisma/adapter-mariadb` 경유 — 직접 `new PrismaClient()` 금지). 입력은 항상 파라미터 바인딩으로 전달하고 문자열 보간(raw SQL 주입)을 피한다. 대체 ORM(TypeORM 등) 도입 금지.
...
- **서버 런타임 자산**(폰트·템플릿·바이너리): `node_modules` 를 런타임에 읽지 않는다 — 번들러 트레이싱 대상이 아니라 standalone/정리된 이미지에서 누락된다. 리포에 커밋한 자산을 `process.cwd()` 기준으로 읽고, 포맷은 **비압축**(TTF > WOFF — 압축 해제 비용이 요청마다 든다).
