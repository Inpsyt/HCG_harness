---
name: codex-review
description: 비가역·고위험 변경이나 릴리스 직전 누적 diff 를 외부 교차모델(codex)로 온디맨드 적대적 리뷰하는 절차 — 닫힌 트리거 7종 판정, 사용자 제안·승인, 백그라운드 실행, D9 핵심(정확성·계약 위반만 액션). 게이트 아님. 트리거 해당 작업을 완료했거나 릴리스를 준비할 때 로드.
---
...
> 게이트로의 퇴화다. 개정하려면 README 변경 이력에 근거를 남긴다(살아있는 문서 규율).
