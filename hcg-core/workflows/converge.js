export const meta = {
  name: 'converge',
  description:
    'Read-only contracts↔code DRIFT reconciliation (Spec Kit /converge analog). Compares the contracts/ SSOT (db-schema · api-spec · shared-types · design-guide) against the implementation, classifies each contract requirement satisfied / partial / missing / contradicts, adversarially verifies each drift to drop false positives, then synthesizes a drift report + PROPOSED tasks for the session to act on. It does NOT write tasks or edit code — every agent runs as the built-in `Explore` agent type (file edits runtime-blocked; shell mutation advisory). Fail-closed: a degraded reconciler/verifier or a missing contracts dir yields `verdict:"incomplete"`, never a false `"aligned"`.',
  whenToUse:
    'Periodically, or before closing a milestone, to detect drift between the authoritative contracts/ and what the code actually implements — the CI drift gate only catches the deterministic part, and nothing measures the semantic gap. Pass an optional scope as args (e.g. "db-schema,api-spec" to limit areas, or a feature path). Output: per-requirement drift (satisfied/partial/missing/contradicts) + proposed reconciliation tasks. Read-only; the session applies the fixes itself (or dispatches independent ones via the `parallel-tasks` skill).',
  phases: [
