...
export function main(argv, deps = {}) {
  ...
  if (args.mode === "init") {
    if (readMarker(args.target, fs) && args.initMode !== "force") {
      log(JSON.stringify({ ok: false, alreadyBootstrapped: true, mode: "init",
        hint: "이미 부트스트랩된 프로젝트입니다(.claude/.hcg-harness.json 존재). 템플릿 갱신은 /hcg-upgrade 를, 재생성을 강제하려면 --force 를 사용하세요." }));
      return 1;
    }
  ...
  if (args.mode === "upgrade") {
    ...
    if (!prev) { log(JSON.stringify({ ok: false, error: "마커가 없습니다. 먼저 /hcg-init 를 실행하세요." })); return 1; }
