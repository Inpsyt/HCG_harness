...
- `<appDir>/package.json` 자체가 없으면(앱 골격이 아예 없는 프로젝트) 이 단계는 건너뛴다.
...
- 앱 골격 생성 여부: **항상 아니오(`--no-app`)** — 이행 재건은 하네스 레이어만 담당한다.
  hcg-core 0.1.4 부터 앱 템플릿은 새 스택 세대(Next 16 · Tailwind 4 · Prisma 7)다. 구 세대 앱
  위에 gap-fill 로 새 세대 파일(`prisma.config.ts` · `lib/db.ts` 등)이 끼어들면 설치되지 않은
  패키지를 import 하는 파손 상태가 된다. 기존 앱(`{{APP_DIR}}/**`)과 그 CI(`.github/`)는 사용자
  소유로 그대로 둔다(0.3.1 부터 철거도 `.github/workflows/ci.yml` 을 지우지 않는다 — 구 앱과
  스택이 정합한 것은 레거시 CI 다). 앱 스택을 hcg 새 표준으로 올리는 것은 이행 뒤의 **별도
  과업**으로 안내한다.
...
(없는 것만 채움 + `--no-app` 이라 `{{APP_DIR}}/**` · `.github/` 는 렌더 대상 자체가 아니다 →
살아남은 `contracts/**` · `.claude/project.md` · `.claude/skills/**` 와 앱 전체가 보존된다).

`created` 목록을 사용자에게 보여준다 — gap-fill 은 사용자가 의도적으로 지웠던 하네스 파일(스킬
스텁 등)도 되살릴 수 있으므로, 원치 않는 파일은 삭제하도록 안내한다.
...
4. `.github/workflows/ci.yml` — 이행은 이 파일을 건드리지 않는다(0.3.1 부터 철거 대상이 아니고,
   재건도 `--no-app` 이라 새로 쓰지 않는다). 앱 골격이 있던 프로젝트라면 **레거시 본이 그대로
   남아 있는지** 확인한다 — 구 앱과 스택이 정합한 CI 다. 앱이 없던 프로젝트는 원래 없음이 정상.
   앱·CI 를 hcg 새 스택 표준으로 올리는 것은 이행 밖의 별도 과업으로 검토 목록에 싣는다.
...
(5단계) · **앱 스택 업그레이드**(구 세대 앱·CI 를 hcg 새 표준 — Next 16 · Tailwind 4 · Prisma 7 —
으로 올리는 별도 과업, 위 4번 항목).
