...
**하네스 의존 없음.** `contracts/`·phase 파일·`.claude/project.md` 가 없는 외부 프로젝트에서도 그대로
동작한다. 있으면 추가 입력으로 쓸 뿐 전제하지 않는다.
...
4. **실 PII 보호.** 화면에 실제 사용자 실명·연락처가 뜨면 스크린샷을 저장하지 않고, 리포트에
   실명을 쓰지 않는다(역할·ID 로 대체).
...
**0 TARGET** — 대상 URL, 범위(스모크 / 특정 플로우 / 전체), 시간 예산을 먼저 합의한다.
범위가 "전체"면 3단계에서 스위트를 나누고 실행 순서를 제시한 뒤 시작한다.
...
**4 RUN** — 두 레이어를 모두 돈다:
- **러너 레이어**: 스택의 단위/통합 테스트 명령을 실행하고 집계를 `layers[]` 에 기록
- **E2E 레이어**: 드라이버로 시나리오를 실행
