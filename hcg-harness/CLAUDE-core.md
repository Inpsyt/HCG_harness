...
이 파일은 `CLAUDE.md` 에서 `@import` 된다 — 프로젝트 불변의 개발 방법론(개발 워크플로우 · fast-path · 공유 계약서 개념 · Task 관리 · Coding Agent Operating Rules §0~§5)을 담는다. 이 문서는 버전 관리되는 **살아있는 문서**다 — 모델 세대·플랫폼 변화로 규칙의 전제가 깨지면 개정한다. §0~§5 는 방법론의 뼈대이므로 개정 시 하네스 레포의 `CHANGELOG.md` 에 근거를 남긴다.

## 개발 워크플로우 (기본)  〔■ HARNESS〕
**모든 Task는 기본적으로 아래 팀 에이전트 파이프라인을 통해 개발한다. 직접 코드 작성은 기본 금지.**

단, **§0 우선순위가 상위 규범이다** — 사용자의 명시 지시는 이 파이프라인 의무를 오버라이드한다(예: 사용자가 직접 수정을 지시하면 파이프라인 없이 수행한다). 오버라이드 시에도 검증 사다리(§4)는 그대로 적용한다.
...
- qa-agent 통과할 때까지 ⑤⑥ 반복 — **상한 2회**: 동일 이슈로 2회 연속 FAIL 시 반복을 멈추고 사용자에게 설계 재검토를 에스컬레이션한다(수렴하지 않는 QA 루프는 개별 버그가 아니라 설계 결함의 신호다)

### 설계 승인 체크포인트 (①.5 — 조건부)

plan-agent 완료 후 오케스트레이터는 설계 요약을 사용자에게 제시한다 — **요약에 포함**: Phase 제목·MoSCoW, `contracts/` 변경 요지(신규/수정 계약), 에이전트별 Task 목록, 주요 가정. 이후 진행은 **§1 판정(가역성 × 비용)** 을 따른다:

- **승인 대기 (하드 블록)** — 설계에 *되돌리기 어렵거나 비싼 결정*이 포함될 때만: `contracts/` 신설·변경, 스키마/마이그레이션, auth·결제·데이터 삭제/이관·외부 부작용. **명시적 승인 후에만** ②③④ 를 호출한다.
- **고지 후 진행 (소프트 블록)** — 위에 해당하지 않으면 요약 제시 후 바로 dispatch 한다(응답 대기 없음). 사용자는 언제든 veto 로 중단·수정시킬 수 있다.
- 사용자가 수정을 요청하면 plan-agent 를 재호출해 계약/Task 를 갱신하고 **다시 제시**한다.
- ⑥의 수정 Task 재발급 루프는 이미 제시된 설계 범위 내이므로 재고지 불요 — 단, 수정이 `contracts/` 를 바꾸면 승인 대기(하드 블록)로 격상된다.
...
- fast-path여도 **직접 코드 수정은 기본 금지**(§0 사용자 오버라이드는 유효) — 단일 담당 에이전트(②③④ 중 하나)로 라우팅하고 그 에이전트가 검증(tsc/lint/build/test)까지 수행한다.
...
**사전 고지 의무 (필수):** fast-path로 가기로 하면 착수 전에 한 줄로 선언한다 — 예: `fast-path: 4게이트 전부 아니오(계약 무변경·단일 backend·설계 결정 없음·되돌리기 쉬움), 새 로직 없어 codex 생략`. **선언 후 응답을 기다리지 않고 진행한다** — 사용자의 거부(veto)가 도착하면 그 시점부터 풀 파이프라인으로 전환한다. **선언 없이 fast-path 금지.** 이 선언은 휘발성이므로 사후 회귀 분석을 위해 `tasks/phase-meta.yml` 의 `fast_path_log:` 에도 한 줄 영속화한다(`pipeline-phase` 스킬).

## 모델 배정 (전 에이전트 세션 모델 상속)  〔■ HARNESS〕

모든 에이전트는 **세션 모델을 상속(inherit)** 한다 — 에이전트 파일은 `model: inherit` 를 유지하고(frontmatter 수정 금지), 오케스트레이터는 spawn 시 `model` 파라미터를 **지정하지 않는다**.

- **근거 (0.2.2 매트릭스 폐지)**: 티어 강등(MoSCoW×난이도 → T1/T2, 0.2.0~0.2.1)은 실패 방향 분석이 반쪽이었다 — 규율을 *놓친* 방향(inherit)은 비용 증가에 그치지만, 규율을 *지킨* 방향(강등)은 품질 저하 → QA 재작업 루프였고, 재작업 비용이 토큰 절감을 상회했다.
- Task 라인의 `난이도: · 티어:` 필드, qa 티어 상향 규칙, `.claude/project.md` 「모델 배정」 해소표는 **폐지** — 기존 프로젝트 문서의 잔존 표기는 무시한다.
- 예외: 경량 모델이 명백히 충분한 대량·기계적 작업은 **워크플로**(audit/migrate 등)의 `model`/`effort` 파라미터로 개별 결정한다 — 워크플로와 codex 게이트(외부 모델)는 종전과 같이 이 체계 밖이다.
