...
이 파일은 `CLAUDE.md` 에서 `@import` 된다 — 프로젝트 불변의 개발 방법론(개발 워크플로우 · fast-path · 공유 계약서 개념 · Task 관리 · Coding Agent Operating Rules §0~§5)을 담는다. 이 문서는 버전 관리되는 **살아있는 문서**다 — 모델 세대·플랫폼 변화로 규칙의 전제가 깨지면 개정한다. §0~§5 는 방법론의 뼈대이므로 개정 시 변경 이력(README)에 근거를 남긴다.
