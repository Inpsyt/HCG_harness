...
- **인프라 의존**: 이 게이트는 소비 프로젝트가 배선한 codex 래퍼(`pnpm codex:review` → `scripts/codex-review.mjs`)와 codex-companion 인증에 의존한다(설치: `docs/install.md`). 미설치/인증만료 시 게이트 실행 불가 → Phase 완료 차단(스킬 §실패 모드 참조).
