...
- **스택·경로·계약 위치·활성 에이전트는 `.claude/project.md` 를 읽고 따른다** (인스턴스 슬롯 단일 출처 — Task 분배·경로 지정 시 참조).
...
- **모델 배정**: 없음 — 모든 에이전트는 세션 모델을 상속(inherit)하며, 티어 매트릭스는 0.2.2 에서 폐지되었다(CLAUDE-core §모델 배정). Task 라인에 `난이도:`·`티어:` 를 기록하지 않는다.
...
#### 최종 보고 — 체크포인트 판정 명시 (①.5 조건부)

Analyze 게이트 통과 후, plan-agent 는 최종 출력으로 **설계 요약**(Phase 제목·MoSCoW, `contracts/` 변경 요지, 에이전트별 Task 목록, 주요 가정)을 보고하고, 마지막 줄에 **체크포인트 판정**을 명시한다(CLAUDE-core §설계 승인 체크포인트 ①.5):

- 설계에 되돌리기 어려운 결정(`contracts/` 신설·변경, 스키마/마이그레이션, auth·결제·데이터 삭제/이관·외부 부작용) 포함 → **`승인 대기 — 사용자 승인 후 구현 dispatch`** (하드 블록).
- 미포함 → **`고지 후 진행 — veto 가능`** (오케스트레이터는 요약 제시 후 바로 dispatch).
