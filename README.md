...
> **주의 — `CLAUDE-core.md` 는 두 벌이다.** 플러그인 루트본과 `profiles/hcg/templates/.claude/`
> 템플릿본이 **바이트 동일**해야 한다. 프로젝트에 렌더되는 것은 템플릿본이므로 루트본만 고치면
> 개정이 도달하지 않는다. 동기화를 강제하는 테스트는 아직 없다(후속 과제).
