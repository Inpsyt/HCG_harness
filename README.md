...
2. 세션 열기 → SessionStart 감지기가 "아직 부트스트랩 안 됨 → /hcg-init 실행" 안내 주입
3. /hcg-init 실행 → 프레임워크 선택(HCG 기본) · 프로젝트명 · 앱 레이아웃 질문
