...
### hcg-core 0.1.6 — 2026-09-17

**적대적 설계 감사 반영 — 수용 2건 · 기각 3건.** 외부 비판("자기 원칙과 구현이 어긋난다" 외
3건)을 저장소 실체(hooks 배선·스킬 본문·설계 스펙 D6/D8·레거시 가드 코드)와 대조 검증한 결과.
사실관계는 4건 모두 정확했으나 처방은 절반만 타당 — 수용/기각 판정 근거는 아래가 정본.

- **PreToolUse 파괴적 명령 가드 이식(수용)** — 레거시 0.3.0 G2 를 독립 모듈로 추출
  (`hooks/destructive-guard.mjs` + 테스트 12건, matcher `Bash|PowerShell`): prisma migrate
  reset · db push 데이터 손실 플래그 · SQL DROP/TRUNCATE · 루트 `rm -rf`/`Remove-Item` ·
  `git push --force`(--force-with-lease 는 허용) 거부, 해제는
  `HARNESS_DISABLE_DESTRUCTIVE_GUARD=1`, fail-open. 근거 2개: ① `run-headless.mjs` 가
  `--dangerously-skip-permissions` 로 돌아 **무인 경로에는 권한 프롬프트 방어선이 없다** —
  이 훅이 유일한 기계 방어선. ② 0.1.0 훅 폐지(D6)에서 계약 잠금·lint·phase-gate 는 근거와
  함께 빠졌지만 파괴 가드는 contracts-guard 번들에 묶여 **근거 기록 없이 같이 빠졌다**(레거시
  0.3.0 은 하루 뒤 이것만 의도적으로 남겼다 — 비대칭). 설계 스펙 자신의 분류(hobble=모든 도로의
  과속방지턱 vs 가드레일=낭떠러지 커브)로 가드레일 — 편집 경로 비용 0, 정상 경로 오탐 사실상 0.
  **계약 잠금(G1/G3)은 이식하지 않는다** — 세션이 계약의 정당한 저자인 소유 모델에서 잠금은
  정상 경로 마찰(센티널 댄스)로 되돌아간다(0.1.0 폐지 근거 유지).
- **parallel-tasks 결합도 판정 기록(수용)** — Task(군)별 병렬/순차 판정과 근거 한 줄을 §4 통합
  보고에 남기도록 스킬 개정. 판정 ③(설계 결정 전파)은 순수 판단이라 기록이 없으면 통합 버그
  발생 시 오분류 여부를 사후에 셀 수 없다. 스킬 로드 시에만 발생하는 조건부 비용이며 기존 통합
  보고의 확장이라 상시 부기가 아니다.
- **기각 3건(근거 기록 — 재론 시 이 항목 참조)**:
  ① *contracts/·auth 경로 PreToolUse 확인 훅* — 훅 페이로드에 에이전트 식별자가 없어(레거시
  contracts-guard.mjs 주석의 실측) "사용자 합의 여부"를 훅이 판별할 수 없고, 세션=정당 저자
  구조에서 센티널 댄스가 정상 경로에 복귀한다(=걷어낸 hobble 의 재도입). 제안의 "해당 없으면
  비용 0" 전제도 부정확 — PreToolUse 는 도구명으로만 매칭돼 모든 편집 호출에 프로세스 스폰이
  붙는다.
  ② *codex 제안 누락 감지 Stop 훅* — Stop 은 세션 종료가 아니라 **매 턴 종료마다** 발화(상시
  비용)하고, 제안 의무는 이미 한 번 실측 반증→§4 완료 절차 편입→라이브 4/4 발동으로 검증됐다
  (0.1.1). 설계 스펙 §9 부활 조건("실사용에서 부재가 증명될 때") 미충족 — 미제안 사례가 실측되면
  재검토한다. "제안 후 거절"은 codex-review §4 완료 보고(실행 여부 명시)로 이미 구분된다.
  ③ *벤치마크 3회 재현·n=1 표기 보강* — 레거시 암 1회 $214·3.5h 로 재현 3회는 폐기 예정 시스템
  방어에 $650+ 소모이며, unhobble 결정의 근거는 벤치마크 단독이 아니라 2026-08-05 진단이
  선행(벤치는 확인 사격)이다. README §2 서술("같은 기능정의서를 … 실측했다")이 단일 실측임을
  이미 드러내므로 표기도 현행 유지.

### hcg-core 0.1.5 — 2026-09-07
- intake-audit 스킬 추가 — UX파트 바이브코딩 프로토타입 인수 진단(살림/수리/재제작 3등급,
  read-only, 컨벤션 조항 인용 의무). 이종 스택은 의도 추출 모드(계약 초안 역추출)로 전환.
  근거: AX기획실 개편 후 UX→프로덕트 인수 공정 표준화.
...
3중 검증). overtimeLog 운영 오픈에서 같은 절차를 하루에 두 번(도커→네이티브 단일화 · 76 운영
이관) 손으로 반복하며 확립한 실측 절차를 스킬화했다. 소비 프로젝트에서 writing-skills TDD 로
제작·검증 후 이식: 베이스라인(스킬 無) 에이전트는 WHAT 은 대체로 알지만 실행 환경 함정(파괴
명령 권한, `!` 프롬프트의 Git Bash 문법, 대화형 `-p` 행 걸림, URL 인코딩)을 전부 밟았고, 스킬
적용 후 두 시나리오(타 프로젝트 이관 · 대상 서버 미기록) 모두 통과했다.

**설계 판단 근거**:

1. **파괴 명령 인간 승인 원칙** — DROP·덮어쓰기 복원·계정 생성은 세션이 실행하지 않고 완성된
   한 줄 명령을 사용자 `!` 실행으로 위임한다. 비가역 조작의 승인 지점을 명령 실행 자체와
   일치시킨다(별도 "진행할까요?" 질문 불요).
2. **파라미터 확정 규칙의 이원화** — 초안은 "인프라 기본값, 다르면 확인" 이었으나 사용자
   지적으로 순환 논리(안 물으면 다른지 모른다)가 드러나 개정했다. 실측 가능한 값(DB 종류·서버
   버전·dev IP·클라이언트 경로)은 묻지 않고 검증, 실측 불가능한 값(운영 DB·WAS — 어디로
   보낼지)은 `project.md`(인스턴스 슬롯) → `.env.production` → 배포 문서/메모리 순 탐색 후
   없으면 반드시 질문. 조직 표준 IP 는 질문의 Recommended 제안값으로만 강등.
   **기각한 대안**: 인프라 값 하드코딩(타 조직 이식성 훼손 + 오서버 복원 위험) · 매번 전부
   질문(모든 모호성마다 묻는 것 자체가 실패 모드 — §1).
3. **검증은 ORM 경로 우선** — parity(`COUNT(*)` diff)·마이그레이션 상태를 DB 클라이언트 직접
   쿼리가 아닌 프로젝트의 Prisma/node 로 수행. 권한 프롬프트 마찰이 적고 프로젝트의 자연스러운
   도구를 쓴다. `information_schema.TABLE_ROWS` 는 근사치라 금지.

실행 함정 표(Git Bash 문법·SOURCE 복원·sandbox 마커·신→구 버전 스큐·cwd busy)는 2026-09-01
overtimeLog 실전에서 전부 실제로 부딪힌 것만 수록했다.
